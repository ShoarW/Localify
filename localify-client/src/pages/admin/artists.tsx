...
import { api, AdminArtist } from "../../services/api";
...
export const AdminArtistsPage = () => {
  ...
  const [artistsData, setArtistsData] = useState<PagedArtists | null>(null);
  ...
  useEffect(() => {
    const fetchArtists = async () => {
      setIsLoading(true);
      try {
        const data = await api.getPagedArtists(currentPage, pageSize);
        setArtistsData(data);
      } catch (error) {
  ...
  const renderPaginationNumbers = () => {
    ...
    if (endPage < artistsData.totalPages) {
      if (endPage < artistsData.totalPages - 1) {
        pages.push(
      ...
      pages.push(
        <button
          key={artistsData.totalPages}
          onClick={() => handlePageChange(artistsData.totalPages)}
          className="px-3 py-1 rounded-lg text-white/60 hover:text-white hover:bg-white/10 transition-colors"
        >
          {artistsData.totalPages}
        </button>
        ...
        <table className="w-full">
          ...
          <tbody>
            {artistsData?.artists.map((artist) => (
              ...
              >
                <td className="px-6 py-4 text-sm text-white/60">{artist.id}</td>
                ...
                <td className="px-6 py-4 text-white">{artist.trackCount}</td>
                <td className="px-6 py-4 text-white">{artist.albumCount}</td>
                <td className="px-6 py-4">
                  <span
      ...
      <div className="px-6 py-4 border-t border-white/10 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="text-sm text-white/60">
            Page {artistsData?.currentPage} of {artistsData?.totalPages}
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => handlePageChange(currentPage + 1)}
              disabled={!artistsData || currentPage === artistsData.totalPages}
              className="p-2 rounded-lg bg-white/5 text-white/60 hover:text-white hover:bg-white/10 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
